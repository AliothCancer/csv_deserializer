...
use std::str::FromStr;
    ...
    pub enum sepal_length_cm { Float(f64), Null }
    ...
    #[derive(Debug, Clone, Copy, PartialEq)]
    pub enum sepal_width_cm { Float(f64), Null }
    ...
    #[derive(Debug, Clone, Copy, PartialEq)]
    pub enum petal_length_cm { Float(f64), Null }
    ...
    #[derive(Debug, Clone, Copy, PartialEq)]
    pub enum petal_width_cm { Float(f64), Null }
...
pub fn new(dataset: &csv_deserializer::CsvDataset) -> Self{
        let (index, _) = dataset
            ...
            .unwrap();
let sepal_length_cm = CsvColumn::sepal_length_cm(dataset.values[index].iter().map(|val| match val{
...
let (index, _) = dataset
            ...
            .unwrap();
let sepal_width_cm = CsvColumn::sepal_width_cm(dataset.values[index].iter().map(|val| match val{
...
let (index, _) = dataset
            ...
            .unwrap();
let petal_length_cm = CsvColumn::petal_length_cm(dataset.values[index].iter().map(|val| match val{
...
let (index, _) = dataset
            ...
            .unwrap();
let petal_width_cm = CsvColumn::petal_width_cm(dataset.values[index].iter().map(|val| match val{
...
let (index, _) = dataset
            ...
            .unwrap();
let target = CsvColumn::target(dataset.values[index].iter().map(|val| match val{
...
pub fn get_columns(&self)-> [&CsvColumn;5] {
        [&self.sepal_length_cm,&self.sepal_width_cm,&self.petal_length_cm,&self.petal_width_cm,&self.target,]
    }}
